...
//
// Same as vision_matmul_q8_sgmat but with fused GELU activation. Single output.
//
...
fn main(@builtin(workgroup_id) wg: vec3<u32>,
  ...
  for (var e = tid; e < WG_M * WG_N; e = e + WG_SIZE) {
    ...
    let gn = tileN + n;
    if (gm < p.M && gn < p.N) {
      let v = shmem_result[m * WG_N + n] + select(0.0, bias[gn], p.hasBias != 0u);
      let inner = 0.7978845608028654 * (v + 0.044715 * v * v * v);
      out[gm * p.N + gn] = 0.5 * v * (1.0 + tanh(clamp(inner, -15.0, 15.0)));
    }
  }
}
